--[[
	Roblox core backpack
	This file has been modified by Neztore to remove dependency on Core APIs which it cannot access and add some additional functionality.
	Key changes:
		* Slots can now be accessed by running BackpackScript:GetSlots()
		* BackpackScript.SlotsChanged is a bindable event which is fired when tool order is updated. It is not passed any parameters. 
		* The backpack is no longer VR compatible. If VR is enabled, the normal UI should be used.
		* The interface will not enlarge for "massive" screens. This feature depended on APIs which not availible to normal scripts.
...
BackpackScript.SlotsChanged = Instance.new('BindableEvent')
...
local GameTranslator = require(script.Parent.GameTranslator)
local Camera = game.Workspace.CurrentCamera
local LocalPlayer = PlayersService.LocalPlayer

local UIParent = Instance.new("ScreenGui")
UIParent.Name = "Backpack"
UIParent.ResetOnSpawn = false;
UIParent.Enabled = true
UIParent.Parent = LocalPlayer:WaitForChild("PlayerGui")
...
local IS_PHONE = UserInputService.TouchEnabled and Camera.ViewportSize.X < HOTBAR_SLOTS_WIDTH_CUTOFF
...
function BackpackScript:GetSlots()
	return Slots;
end
...
local function EvaluateBackpackPanelVisibility(enabled)
    -- Original: return enabled and StarterGui:GetCoreGuiEnabled(Enum.CoreGuiType.Backpack) and TopbarEnabled and VRService.VREnabled
...
BackpackScript.FindLowestEmpty = FindLowestEmpty;
		...
		SlotFrame.DragStopped:connect(function(x, y)
			...
			if CheckBounds(InventoryFrame, x, y) then
				if slot.Index <= NumberOfHotbarSlots then
					...
					BackpackScript.SlotsChanged:Fire()
				end
...
local function OnChildAdded(child) -- To Character or Backpack
	...
	if slot then
		...
		slot = BackpackScript.getSlotCallback(tool) or LowestEmptySlot or MakeSlot(UIGridFrame)
		slot:Fill(tool)
...
function changeSlot(slot)
	...
	if slot.Frame == SelectedObject and swapInVr then
		...
		if currentlySelectedSlot then
			...
			if currentlySelectedSlot ~= slot then
				...
				if slot.Index > NumberOfHotbarSlots and not slot.Tool then
					if SelectedObject == slot.Frame then
						SelectedObject = currentlySelectedSlot.Frame
					end
					slot:Delete()
				end

				if currentlySelectedSlot.Index > NumberOfHotbarSlots and not currentlySelectedSlot.Tool then
					if SelectedObject == currentlySelectedSlot.Frame then
						SelectedObject = slot.Frame
					end
...
function enableGamepadInventoryControl()
	...
	if not UseGazeSelection() then
		SelectedObject = HotbarFrame:FindFirstChild("1")
	end
end
...
function disableGamepadInventoryControl()
	...
	if SelectedObject and SelectedObject:IsDescendantOf(MainFrame) then
		SelectedObject = nil
	end
end
...
do -- Make the Inventory expand/collapse arrow (unless TopBar)
	local removeHotBarSlot = function(name, state, input)
		...
		for i = 1, NumberOfHotbarSlots do
			if Slots[i].Frame == SelectedObject and Slots[i].Tool then
